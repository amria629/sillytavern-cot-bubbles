...
const renderChat = () => {
    ...
    // 1. 【修复图片】必须使用 innerHTML 来保留 <img> 等标签
    const rawHTML = dataContainer.innerHTML;
    
    // 2. 按换行符分割成行数组
    const lines = rawHTML.split(/\r?\n/);

    let currentBubbleContent = [];
    let currentBubbleType = null; // 'left' or 'right'

    // 辅助函数：用于将当前缓冲区的内容生成一个气泡并清空
    const flushBubble = () => {
        if (currentBubbleContent.length > 0) {
            // 将聚合的多行内容用换行符连接起来
            const content = currentBubbleContent.join('\n');
            addMessage(content, currentBubbleType === 'right');
        }
        currentBubbleContent = []; // 清空缓冲区
    };
    ...
    lines.forEach(line => {
        ...
        // 如果遇到空行，说明一个逻辑块结束，立即“刷出”当前的气泡
        if (trimmedLine === '') {
            flushBubble();
            currentBubbleType = null; // 重置类型，让下一行内容开启新气泡
            return; // 继续处理下一行
        }

        // 判断当前行的类型
        const lineType = (trimmedLine.startsWith('[') && trimmedLine.endsWith(']')) ? 'right' : 'left';
        
        // 如果类型改变了，先“刷出”旧的气泡
        if (lineType !== currentBubbleType && currentBubbleType !== null) {
            flushBubble();
        }

        // 设置当前气泡的类型
        currentBubbleType = lineType;
        
        // 将处理过的内容添加到缓冲区
        if (lineType === 'right') {
            // 右气泡内容需要去掉方括号
            currentBubbleContent.push(trimmedLine.slice(1, -1).trim());
        } else {
            // 左气泡内容原样保留（这样<img>标签就能生效）
            currentBubbleContent.push(line); 
        }
    });
    
    // 4. 循环结束后，别忘了“刷出”最后一个气泡
    flushBubble();
